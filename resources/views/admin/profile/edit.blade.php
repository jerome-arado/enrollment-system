...
<div class="page-sm">
    ...
    <div class="section-header">
        <h1>Admin Profile</h1>
        <p>Update your account information, profile picture, and security.</p>
    </div>
    ...
    <div class="card">
        ...
        <p class="text-muted text-sm" style="margin-bottom:1.5rem;">
            This photo appears in the navigation bar.
        </p>
        ...
        <div class="avatar-edit-row">
            ...
            <div class="avatar-display" id="avatarDisplay">
                @php
                    // For admin, store profile picture path in a separate user field? 
                    // We'll use a profiles/admin/{user_id} convention.
                    $adminPic = auth()->user()->profile_picture ?? null;
                    $initials = collect(explode(' ', $user->name))
                        ->map(fn($w) => strtoupper(substr($w, 0, 1)))
                        ->take(2)->implode('');
                @endphp

                @if($adminPic && Storage::disk('public')->exists($adminPic))
                    <img src="{{ asset('storage/' . $adminPic) }}"
                         alt="Profile picture"
                         id="avatarPreview"
                         class="avatar-large">
                @else
...
function handleFileSelect(input) {
    ...
    document.getElementById('fileSize').textContent = sizeKb > 1024 ? sizeMb + ' MB' : sizeKb + ' KB';
...
function cancelSelect() {
    ...
    // Revert preview to stored picture or initials
    @php
        $storedPic = auth()->user()->profile_picture ?? null;
        $hasStoredPic = $storedPic && Storage::disk('public')->exists($storedPic);
    @endphp
    @if(isset($storedPic) && $hasStoredPic)
        const preview = document.getElementById('avatarPreview');
        const initials = document.getElementById('avatarInitials');
        preview.src = "{{ asset('storage/' . $storedPic) }}";
        preview.style.display = 'block';
        if (initials) initials.style.display = 'none';
    @else
        const preview = document.getElementById('avatarPreview');
